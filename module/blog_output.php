...
if ($newsDataId > 0) {
    ...
    if (count($datas)) {
				...
				if (rex_addon::get('lazyload')->isAvailable()) {
					$blogbeitrag .= '
						<figure itemprop="associatedMedia" itemscope itemtype="http://schema.org/ImageObject" class="col-md-3 col-sm-6 gallery-item">
							<a href="'.rex_url::base('index.php?rex_media_type=photoswipe&rex_media_file='.$file.'').'" itemprop="contentUrl" data-size="'.$width.'x'.$height.'">
							<img src="'.rex_url::base('index.php?rex_media_type=lazyimage&rex_media_file='.$file.'').'" width="100%" height="100%" itemprop="photoswipe_thumbnail" alt="'.$description.'" data-src="'.rex_url::base('index.php?rex_media_type=photoswipe_thumb&rex_media_file='.$file.'').'" class="b-lazy img-rounded img-responsive" /></a>
					...
					$blogbeitrag .= '
						<figure itemprop="associatedMedia" itemscope itemtype="http://schema.org/ImageObject" class="col-md-3 col-sm-6 gallery-item">
							<a href="'.rex_url::base('index.php?rex_media_type=photoswipe&rex_media_file='.$file.'').'" itemprop="contentUrl" data-size="'.$width.'x'.$height.'">
							<img src="'.rex_url::base('index.php?rex_media_type=photoswipe_thumb&rex_media_file='.$file.'').'" itemprop="photoswipe_thumbnail" alt="'.$description.'" class="img-rounded img-responsive" /></a>
							<figcaption itemprop="caption description" class="gallery-caption">'.$description.'</figcaption>
						</figure>';				}
		...
		<div class="youtube"><iframe src="https://www.youtube-nocookie.com/embed/' . $data['youtube'] . '?rel=0&amp;showinfo=0" width="100%" height="100%" frameborder="0" webkitAllowFullScreen mozallowfullscreen allowFullScreen></iframe></div>';
			...
			<a class="btn btn-icon btn-fill btn-twitter" href="https://twitter.com/intent/tweet?text=' . preg_replace ( '/[^a-z0-9_ßöäüÖÄÜ ]/i', '', $data['title'] ) . '&url=http://' . $_SERVER['HTTP_HOST'] . rex_getUrl($newsArticleId, '', ['id' => $data['id']]) . '&via=greatif_media"><i class="fa fa-twitter"></i></a>
	...
	// Wenn ein YCom-User angemeldet ist (dann wird das verkürzte Kommentar-Formular ausgegeben)
	if ($ycom_user) {
		...
		if ($anzahl_user == '0') {$anzahl_user = '1';}
		
		$anzahl_comments = $sql->getRows() / $anzahl_user;

		// Kommentar-Block
		$kommentare .= '
		<div class="section section-nude-gray" id="comments">
			<div class="media-area">
				<h3 class="text-center">Kommentare ('.$anzahl_comments.')</h3>
				<hr>';

				for ($i=0; $i<$sql->getRows(); $i++) {

					// Wenn die User-ID des Kommentars nicht leer ist, d.h. der Kommentar von einem Community-User stammt, und User-ID des Users mit der User-ID des Kommentars übereinstimmmt					
					if ($sql->getValue('user_id') != '' AND $sql->getValue('rex_ycom_user.id') == $sql->getValue('user_id')) {
					...
					// Wenn die User-ID des Kommentars leer ist, d.h. der Kommentar nicht von einem Community-User stammt, und die User-ID des Users "1" ist (damit diese Kommentare nicht pro existierendem User mehrfach gezählt werden)
					if ($sql->getValue('user_id') == '' AND $sql->getValue('rex_ycom_user.id') == '1') {
						$profile_pic = 'profile_default.png';
							$kommentare .= '
								<div class="media">
		                          <div class="avatar">
	...
	if ($sql2->getValue('anzahl') > '0') {
		...
		<div class="section section-nude-gray" id="comments">
			<div class="media-area">
				<h3 class="text-center">'.$anzahl_comments.' Kommentare</h3>
				<hr>';
	...
	if ($sql2->getValue('anzahl') == '0') {
		...
		$yform->setValueField('ip', array("ip","IP","1"));
		...
		$yform->setValueField('html', array('opendiv','openDIV','<div id="customizeddiv" class="col-xs-12 col-sm-12 col-md-12" style="padding: 5px;">'));
		$yform->setValueField('checkbox', array('checkbox_dsgvo','Ich habe die <a href="'.$datenschutz_url.'" target="_blank">Datenschutzerklärung</a> zur Kenntnis genommen. Ich stimme zu, dass meine Angaben und Daten elektronisch erhoben und gespeichert werden.','0','1'));
		$yform->setValueField('html', array('closediv','closeDIV','</div>'));
		...
		// ggf. Kommentar auch an Admin senden:
		   $yform->setActionField('email', array('noreply@greatif.de', 'kontakt@greatif.de', 'Neuer Kommentar von ###vorname### ###nachname###', '###ip### <br /> ###emailadresse### <br /> ###comment###'));
